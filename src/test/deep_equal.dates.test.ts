...
// dates: what SHOULD be equal (these tests pass)
test_equal_values([
	['same date reference', date1, date1],
	['dates with same timestamp', date1, date2],
	['dates from same string', new Date('2024-01-15'), new Date('2024-01-15')],
	['dates from same timestamp number', new Date(1705315800000), new Date(1705315800000)],
	['invalid date instances', new Date('invalid'), new Date('invalid')], // both NaN
	['epoch date instances', new Date(0), new Date(0)],
]);

// dates: what SHOULD NOT be equal
test_unequal_values([
	// type mismatches
	...
	['dates with different timestamps', new Date('2024-01-15'), new Date('2024-01-16')],
